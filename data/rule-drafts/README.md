...
Two drafts remain, both `detect: rendered` in the source: `interaction-layout-shift` (content that moves after paint) and `typography-long-content-safety` (overflow and truncation of real text). A regex over source cannot see either; they wait for the rendered extractor (`slop-cop lint --url`) to carry layout deltas and overflow. Every other draft the script once produced has been hand-ported to a regex-plus-`absent` rule under `data/rules/` or authored as its own rule.
